(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))n(e);new MutationObserver(e=>{for(const a of e)if(a.type==="childList")for(const r of a.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&n(r)}).observe(document,{childList:!0,subtree:!0});function s(e){const a={};return e.integrity&&(a.integrity=e.integrity),e.referrerPolicy&&(a.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?a.credentials="include":e.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function n(e){if(e.ep)return;e.ep=!0;const a=s(e);fetch(e.href,a)}})();class v{constructor(t){this.parent=t}getHTML(t){return`
            ...
            `}render(t,s,n,e){const a=this.getHTML(t);this.parent.insertAdjacentHTML("beforeend",a);const r=this.parent.lastElementChild,o=r.querySelector(".details-btn"),h=r.querySelector(".delete-btn"),p=r.querySelector(".edit-btn");o.addEventListener("click",s),h.addEventListener("click",n),p.addEventListener("click",e)}}class u{constructor(t){this.parent=t}addListeners(t){document.getElementById("back-button").addEventListener("click",t)}addListeners1(t){document.getElementById("filter-button").addEventListener("click",t)}addListeners2(t){document.getElementById("add-zap").addEventListener("click",t)}getHTML(){return`
                <header class="shapk">
            ...
            `}clickBack(){new d(this.parent).render()}async handleAddVacancy(){var o,h,p,b;const t=(o=document.getElementById("srcInput"))==null?void 0:o.value.trim(),s=(h=document.getElementById("titleInput"))==null?void 0:h.value.trim(),n=parseInt((p=document.getElementById("zpInput"))==null?void 0:p.value),e=(b=document.getElementById("detaliInput"))==null?void 0:b.value.trim();if(!t||!s||isNaN(n)||!e){alert("Пожалуйста, заполните все поля корректно!");return}const a=new Intl.NumberFormat("ru-RU").format(n),r={id:this.generateId(),src:t,title:s,zp:n,text:`от ${a} ₽ на руки`,detali:e,code:this.generateId()};try{await c.post(l.addJob(),r),this.showSuccessNotification(),await this.returnToMainPage()}catch(m){console.error("Ошибка:",m),alert(`Ошибка сохранения: ${m.message}`)}}async returnToMainPage(){try{const t=new d(this.parent);for(;this.parent.firstChild;)this.parent.removeChild(this.parent.firstChild);await t.render()}catch(t){console.error("Ошибка возврата на главную:",t),location.reload()}}generateId(){let t=1;return this.existingCards.forEach(s=>{const n=parseInt(s==null?void 0:s.id);!isNaN(n)&&n>t&&(t=n)}),console.log(t+1),t+1}showSuccessNotification(){alert("Вакансия успешно добавлена!")}render(){this.parent.innerHTML="",new u(this.parent).render(this.clickBack.bind(this));const s=this.getHTML();this.parent.insertAdjacentHTML("beforeend",s),document.getElementById("add-zap").addEventListener("click",()=>{this.handleAddVacancy()}),new g(this.parent).render()}}class T{constructor(t,s,n,e){this.parent=t,this.onAddCallback=s,this.data=e||[],this.ID=n}getHTML(){const t=this.data.filter(s=>s.id===this.ID);return`
                <div id="main-page" class="d-flex flex-column"><div/>
                <div class="Inp">
                    <dl class="text">
                        <dt class="opis">Картинка</dt>
            ...
            `}FoundIndex(t){for(let s=0;s<=this.data.length;s++)if(this.data[s].id==t)return s;return-1}clickCard(t){const s=t.target.dataset.id,n=this.data.find(e=>e.id==s);n?new w(this.parent,s,n).render():console.error("Вакансия не найдена")}clickADD(){new x(this.parent,s=>{this.data.push(s),console.log("Данные после добавления:",this.data);const n=this.pageRoot.querySelector(".cards-container");n.innerHTML="",this.renderData(this.data,n)},this.data).render(),console.log("Данные после добавления:",this.data)}clickEDIT(t){new T(this.parent,n=>{this.data=this.data.filter(a=>a.id!==n.id),this.data.push(n),console.log("Данные после добавления:",this.data);const e=this.pageRoot.querySelector(".cards-container");e.innerHTML="",this.renderData(this.data,e)},parseInt(t.target.dataset.id),this.data).render(),console.log("Данные после добавления:",this.data)}clickDEL(t){const s=parseInt(t.target.dataset.id);if(isNaN(s))return;this.data=this.data.filter(e=>e.id!==s),c.delete(l.removeJobById(s),()=>{console.log("Удалено с сервера")});const n=this.pageRoot.querySelector(".cards-container");n&&this.renderData(this.data,n)}countIdentic(){let t=0,s,n=[],e={};for(let a=0;a<this.data.length;a++)n.push(this.data[a].code);for(let a=0;a<n.length;a++)s=n[a],e[s]?e[s]++:e[s]=1;for(let a in e)e[a]>1&&(t+=e[a]);return t}srZnach(){let t=0;if(this.data.length>0){for(let s=0;s<this.data.length;s++)t+=this.data[s].zp;return t/this.data.length}else return 0}diapazon(){let t=[];if(this.data.length>0){for(let e=0;e<this.data.length;e++)t.push(this.data[e].id);t.sort((e,a)=>e-a);let s=t[0],n;for(let e=1;e<t.length;e++)t[e-1]+1===t[e]?n=!0:n?(s+="-"+t[e-1]+","+t[e],n=!1):s+=","+t[e];return n&&(s+="-"+t[t.length-1]),s}else return 0}palindrome1(t){let s=t.toLowerCase(),n=s.length-1;for(let e=0;e<n/2;e++){let a=s[e],r=s[n-e];if(a!=r)return"нет"}return"да"}palindrome2(t){let s=t.toLowerCase(),n="";for(let e=s.length-1;e>=0;e--)n+=s[e];return n===s?"да":"нет"}foundFilter(){const s=document.getElementById("searchInput").value.toLowerCase();console.log(s),c.get(l.getJobs(),n=>{const e=n.filter(r=>r.title.toLowerCase().includes(s));this.pageRoot.querySelector(".cards-container").innerHTML="";const a=this.pageRoot.querySelector(".cards-container");this.renderData(e,a)})}async render(){this.parent.innerHTML="";const t=this.getHTML(this.countIdentic(),this.srZnach(),this.diapazon(),this.palindrome1("Привет"),this.palindrome2("Приветтевирп"));this.parent.insertAdjacentHTML("beforeend",t),new u(this.pageRoot).render(this.clickCard.bind(this));const n=await this.getData();await this.renderData(n),document.getElementById("add-zap").addEventListener("click",this.clickADD.bind(this)),document.getElementById("searchInput").addEventListener("input",this.foundFilter.bind(this)),new g(this.pageRoot).render()}}const k=document.getElementById("root"),E=new d(k);E.render();
